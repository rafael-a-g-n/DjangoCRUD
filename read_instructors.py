"""Script to read and query instructors from the database."""
...
instructor_yan = Instructor.objects.get(first_name="Yan")  # pylint: disable=no-member
...
try:
    instructor_andy = Instructor.objects.get(first_name="Andy")  # pylint: disable=no-member
except Instructor.DoesNotExist:
    print("2. Try to find a non-existing instructor with first name `Andy`")
    print("Instructor Andy doesn't exist")

print("\n")
part_time_instructors = Instructor.objects.filter(full_time=False)  # pylint: disable=no-member
print("3. Find all part time instructors: ")
print(part_time_instructors)

print("\n")
full_time_instructors = (  # pylint: disable=no-member
...
print("\n")
full_time_instructors = Instructor.objects.filter(  # pylint: disable=no-member
